...
object ExportUtil {
    ...
    data class ImportedSession(
        val name: String,
        val messages: List<com.netment.hermespocket.data.MessageEntity>
    )
    ...
    fun importSessionJson(
        ...
        return try {
            ...
            val entities = mutableListOf<com.netment.hermespocket.data.MessageEntity>()
            for (i in 0 until msgsArr.length()) {
                val obj = msgsArr.getJSONObject(i)
                val type = obj.optString("type", "chat_msg")
                val entity = when (type) {
                    "chat_msg" -> com.netment.hermespocket.data.MessageEntity(
                    ...
                    "approval_item" -> com.netment.hermespocket.data.MessageEntity(
                        sessionId = 0,
                        type = "approval_item",
